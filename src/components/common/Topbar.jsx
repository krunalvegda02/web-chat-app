    ...
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-[100] shadow-[0_-1px_3px_rgba(0,0,0,0.08)]" style={{ backgroundColor: theme?.backgroundColor || '#F0F2F5' }}>
        <div className="flex items-center justify-around h-14 px-1">
          {menuItems.length > 4 ? (
            // When more than 4 items: Show first 3 + More button
            <>
            ...
            // When 4 or fewer items: Show menu items + Profile + Logout buttons
            <>
              {menuItems.map((item) => {
                const isActive = isMenuItemActive(item.path);
                return (
              ...
              >
                ...
                <span style={{ fontSize: '10px', color: theme?.sidebarTextColor || '#667781', fontWeight: 400, letterSpacing: '0.2px' }}>Profile</span>
              </button>
              {/* Logout button */}
              <button
              ...
              >
                <div className="flex items-center justify-center w-12 h-8 rounded-full">
                  <LogoutOutlined style={{ fontSize: '20px', color: '#ef4444' }} />
                </div>
                <span style={{ fontSize: '10px', color: '#ef4444', fontWeight: 400, letterSpacing: '0.2px' }}>Logout</span>
              </button>
            </>
          )}
        </div>
      ...
      {/* DRAWER - WhatsApp Style - Only for additional menu items when >4 items */}
      {menuItems.length > 4 && (
        <Drawer
          title={null}
          placement="bottom"
        ...
        >
          <div className="flex flex-col bg-white" style={{ borderRadius: '16px 16px 0 0' }}>
            ...
            {/* Profile and Logout actions in drawer for >4 items */}
